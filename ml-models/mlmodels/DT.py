...
def DecisionTree(self, verbose=True, output_plots=True):
    """
    Hyperparameter search for Decision Tree model, returns optimal hyperparameters
    """
    if verbose:
        print("DecisionTree from DT.py is working!")

    tree = DecisionTreeClassifier(random_state=0) #n_jobs = number of jobs to run in parallel, -1 means use all processors
    ...
    #Searches for best parameters for precision
        ...
        #use predict_proba function to calculate average_precision_score
    ...
    #Uses decision function
        ...
        for key, df in self.input_df.items():
        #Grid search with stratified cross validation
        #use predict_proba to calculate roc_auc
            skfold = StratifiedKFold(n_splits=5)
