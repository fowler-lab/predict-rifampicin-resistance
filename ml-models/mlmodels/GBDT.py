...
def GradientBoostedDecisionTree(self, verbose=True, output_plots=True):
    """
    Hyperparameter search for Gradient Boosted Decision Tree model, returns optimal hyperparameters
    """

    if verbose:
        print("GBDecisionTree from GBDT.py is working!")
    ...
    if self.scoring_metric == "accuracy":
        ...
        for key, df in self.input_df.items():
            ...
            grid_strat = GridSearchCV(tree, param_grid, cv=skfold, scoring='accuracy', refit=True)
            ...
            grid_kfold = GridSearchCV(tree, param_grid, cv=kfold, scoring='accuracy', refit=True)
            ...
            grid_kfold_shuffle = GridSearchCV(tree, param_grid, cv=kfold_shuffle, scoring='accuracy', refit=True)
    ...
    #Searches for best parameters for precision
        ...
        #use predict_proba function for positive class to calculate average_precision_score
            ...
            skfold = StratifiedKFold(n_splits=5)
            grid_strat = GridSearchCV(tree, param_grid, cv=skfold, scoring='average_precision', refit=True)
            grid_strat.fit(self.X_train_nd[key], self.y_train_nd[key])
            ...
            #Grid search with kfold cross validation
            kfold = KFold(n_splits=5)
            grid_kfold = GridSearchCV(tree, param_grid, cv=kfold, scoring='average_precision', refit=True)
            grid_kfold.fit(self.X_train_nd[key], self.y_train_nd[key])
            ...
            #Search with shuffled cross validation
            kfold_shuffle=KFold(n_splits=5, shuffle=True, random_state=0)
            grid_kfold_shuffle = GridSearchCV(tree, param_grid, cv=kfold_shuffle, scoring='average_precision', refit=True)
            grid_kfold_shuffle.fit(self.X_train_nd[key], self.y_train_nd[key])
        ...
        Analysis.pr_curve_on_training_set_for_all(self, opt_estimator, opt_features, verbose=False, output_plots=output_plots) #N.B. Includes metrics with shifted thresholds
    ...
    #Uses predict_proba
        ...
        for key, df in self.input_df.items():
        #Grid search with stratified cross validation
        #use decision function to calculate roc_auc
            skfold = StratifiedKFold(n_splits=5)
            grid_strat = GridSearchCV(tree, param_grid, cv=skfold, scoring='roc_auc', refit=True)
            grid_strat.fit(self.X_train_nd[key], self.y_train_nd[key])
            ...
            #Grid search with kfold cross validation
            kfold = KFold(n_splits=5)
            grid_kfold = GridSearchCV(tree, param_grid, cv=kfold, scoring='roc_auc', refit=True)
            grid_kfold.fit(self.X_train_nd[key], self.y_train_nd[key])
            ...
            #Search with shuffled cross validation
            kfold_shuffle=KFold(n_splits=5, shuffle=True, random_state=0)
            grid_kfold_shuffle = GridSearchCV(tree, param_grid, cv=kfold_shuffle, scoring='roc_auc', refit=True)
            grid_kfold_shuffle.fit(self.X_train_nd[key], self.y_train_nd[key])
        ...
        #Reruns ML model with optimal hyperparameters and plots ROC curve
        opt_estimator.fit(self.X_train_nd[opt_features], self.y_train_nd[opt_features])
    ...
    #Searches for best parameters for recall
        ...
        for key, df in self.input_df.items():
            #Grid search with stratified cross validation
            skfold = StratifiedKFold(n_splits=5)
            grid_strat = GridSearchCV(tree, param_grid, cv=skfold, scoring='recall', refit=True)
            grid_strat.fit(self.X_train_nd[key], self.y_train_nd[key])
            ...
            #Grid search with kfold cross validation
            kfold = KFold(n_splits=5)
            grid_kfold = GridSearchCV(tree, param_grid, cv=kfold, scoring='recall', refit=True)
            grid_kfold.fit(self.X_train_nd[key], self.y_train_nd[key])
            ...
            #Search with shuffled cross validation
            kfold_shuffle=KFold(n_splits=5, shuffle=True, random_state=0)
            grid_kfold_shuffle = GridSearchCV(tree, param_grid, cv=kfold_shuffle, scoring='recall', refit=True)
            grid_kfold_shuffle.fit(self.X_train_nd[key], self.y_train_nd[key])
        ...
        ###Generate recall score with max parameters
        opt_features = list(scores["df_name"])[j]
        opt_algorithm = list(scores["search_algorithm"])[j]
        opt_estimator = list(scores["best_estimator"])[j]
        max_cv_score = list(scores["cross_validation_score"])[j]
